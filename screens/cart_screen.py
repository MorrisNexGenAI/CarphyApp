...
class CartScreen(Screen):
    ...
    def update_cart(self):
        layout = self.ids.cart_layout.children[0]  # Target inner BoxLayout
        layout.clear_widgets()
        course = self.manager.get_screen("course").selected_course
        layout.add_widget(Label(text=f"Assignment for {course} (100 LD/sheet)", size_hint_y=None, height=50))
        ...
        layout.add_widget(self.questions_input)
        layout.add_widget(self.instructions_input)
        layout.add_widget(submit_btn)
        self.show_pending_orders()

    def show_pending_orders(self):
        layout = self.ids.cart_layout.children[0]
        app = App.get_running_app()
        orders = get_orders(app.current_user[4])  # Assuming user_id at index 4
